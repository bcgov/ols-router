...
import ca.bc.gov.app.router.util.UrlCsvInputSource;
...
public class FileRouterDataSource implements RouterDataSource {
	...
	private RowReader turnCostReader;
...
//			if(segmentReader.next()) {
			...
			Integer maxWeight = segmentReader.getInteger("vehicle_max_weight");
...
//			AccessRestriction accessRestriction = AccessRestriction.convert(segmentReader.getString("access_restriction_ind"));
			...
			StreetSegment segment = new StreetSegment(segmentId, centerLine,  
					...
					speedLimit, surfaceType, maxHeight, maxWidth, maxWeight, isTruckRoute,
					highwayRoute1, highwayRoute2, highwayRoute3,
					isDeadEnded); //, laneRestriction, accessRestriction
//				if(segment.isFerry()) {
...
//				}
	...
	@Override
	public TurnCost getNextTurnCost() throws IOException {
		if(turnCostReader.next()) {
			String idSeq = turnCostReader.getString("EDGE_NODE_SET");
			byte cost = (byte)turnCostReader.getInt("TRAVERSAL_COST");
	...
	private RowReader getTurnCosts() {
		return getXsvRowReader("turn_costs");
	}
