...
public class RestrictionLookupBuilder {
	...
	private BasicGraphBuilder graphBuilder;
	...
	public RestrictionLookupBuilder(BasicGraphBuilder graphBuilder) {
	...
	public void addRestriction(Restriction r) {
		if(r == null) return;
		int[] edgeIds = graphBuilder.getEdgesForSegment(r.segmentId);
		if(edgeIds == null) {
		...
		if(r.azimuth < 0) {
			...
			LineString ls = graphBuilder.getGraph().getLineString(edgeIds[0]);
	...
	private void buildLaneRestrictions() {
		...
		for(Entry<Integer, List<Restriction>> entry : laneBasedRestrictions.entrySet()) {
			...
			for(List<Restriction> restrictions : locationMap.values()) {
				...
				int numLanes = graphBuilder.getGraph().getNumLanes(edgeId);
				...
				LineString seg = graphBuilder.getGraph().getLineString(edgeId);
	...
	public RestrictionLookup build() {
		...
		return new RestrictionLookup(restrictionMap);
	}
