...
exports.ZodMiniCreditCard = core.$constructor("ZodMiniCreditCard", (inst, def) => {
...
function partial(schema, mask) {
    ...
    return util.clone(schema, {
        ...def,
        items: def.items.map((item) => new exports.ZodMiniOptional({ type: "optional", innerType: item })),
    });
}
// @__NO_SIDE_EFFECTS__
function exactPartial(schema, mask) {
    return util.partial(exports.ZodMiniExactOptional, schema, mask, "exactPartial");
}
// @__NO_SIDE_EFFECTS__
